      ...
      {result &&
          ...
          result.items.map((movie) => (
            <div onClick={() => navHandler(movie.id)} className="w-32 p-3 bg-light cursor-pointer text-dark" key={movie.id}>
              <img className="img-fluid" src="https://gumlet.assettype.com/newslaundry%2F2021-09%2F6c68a48b-f212-4fbf-a0d9-d447dbdd9287%2FDaniel_Craig.jpg?auto=format%2Ccompress&fit=max&format=webp&w=480&dpr=2.6" height={'150px'}/>
              <h3>{movie.name} <span className="text-muted">({movie.year})</span></h3>
              <h4>{movie.avgRating}</h4>
